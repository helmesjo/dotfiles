...
# Do not put && / cd / exec in the payload: msys2_shell.cmd is parsed by
# cmd.exe first, which splits on && and then tries to run ./setup.sh itself.
$argList = @('-ucrt64', '-defterm', '-here', '-no-start', '-use-full-path', '-shell', 'bash', '-c', './setup.sh')
...
# -here + WorkingDirectory puts bash in the repo; no POSIX path rewrite.
$p = Start-Process -FilePath "$Msys2Root\msys2_shell.cmd" -WorkingDirectory $RepoRoot -ArgumentList $argList -NoNewWindow -PassThru -Wait
